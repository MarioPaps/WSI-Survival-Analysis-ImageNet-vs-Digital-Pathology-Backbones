...
def main(args):
    ...
    for i in range(folds):
        ...
        train_dataset, val_dataset, test_dataset = dataset.return_splits(args.backbone, args.patch_size, from_id=False, 
                csv_path='{}/blca_{}.csv'.format(args.split_dir, i)) #blca kirc lusc
    ...
    if (folds) != args.k: ##len(folds) != args.k:
        save_name = 'summary_partial_{}_{}.csv'.format(start, end)
    else:
        #save_name = 'summary.csv'
        save_name= args.exp_code + '.csv'
    print("Saving results to: ", os.path.join(args.results_dir, save_name))
...
parser.add_argument('--patience', type=int, default=20, help='Number of epochs to wait for validation loss to improve')
parser.add_argument('--stop_epoch', type=int, default=20, help='Earliest epoch possible for stopping')
...
if 'survival' in args.task:
    ...
    csv_name= 'dataset_csv/final_df.csv'
    print('Arguments in the function')
    print('csv_name:', csv_name)
    print('mode:', args.mode)
    print('apply_sig:', args.apply_sig)
    print('data_dir:', os.path.join(args.data_root_dir, study_dir))
    print('shuffle:', False)
    print('seed:', args.seed)
    print('print_info:', True)
    print('patient_strat:', False)
    print('n_bins:', 4)
    print('label_col:', 'survival_months')
    print('ignore:', [])
    dataset = Generic_MIL_Survival_Dataset(csv_path = csv_name,
                                            mode = args.mode,
